...
class UNetBlock2D(nn.Module):
    def __init__(
        ...
        if prev_out_channels is None:
            ...
            in_channels_list = [
                a + b for a, b in zip(in_channels_list, res_channels_list, strict=False)
            ]
    ...
    def __init__(self, config: UNetConfig):
        ...
        self.down_blocks = [
            ...
            for i, (in_channels, out_channels) in enumerate(
                zip(block_channels, block_channels[1:], strict=False)
            )
        ]
        ...
        self.up_blocks = [
            ...
            for i, (in_channels, out_channels, prev_out_channels) in reversed(
                list(
                    enumerate(
                        zip(block_channels, block_channels[1:], block_channels[2:], strict=False)
                    )
                )
            )
        ]
