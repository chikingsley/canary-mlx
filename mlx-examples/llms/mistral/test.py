...
class TestMistral(unittest.TestCase):
    ...
    def test_generate(self):
        ...
        tokens = [t for t, _ in zip(mistral.generate(prompt, model), range(30), strict=False)]
        mx.eval(tokens)
    ...
    def benchmark(self):
        ...
        # warmup
        for _ in range(2):
            tokens = [t for t, _ in zip(mistral.generate(prompt, model), range(101), strict=False)]
            mx.eval(tokens)
        ...
        for _ in range(its):
            ...
            mx.eval(next(generator))
            tic = time.time()
            tokens = [t for t, _ in zip(generator, range(100), strict=False)]
            mx.eval(tokens)
