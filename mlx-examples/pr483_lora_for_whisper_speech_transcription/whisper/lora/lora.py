...
def evaluate(model, dataset, loss, tokenizer, batch_size, num_batches):
    ...
    for it, batch in zip(
        range(num_batches),
        iterate_batches(dataset, tokenizer, batch_size), strict=False,
    ):
...
def train(model, loss, tokenizer, args):
    ...
    for it, batch in zip(
        range(args.iters),
        iterate_batches(train_set, tokenizer, args.batch_size, train=True), strict=False,
    ):
...
def freeze_and_lora(args, model):
    ...
    print("Applying LoRA parameters to AudioEncoder...")
    ...
    print("Applying LoRA parameters to TextDecoder...")
