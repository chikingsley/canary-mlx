    ...
    language_probs = [
        {
            ...
            for j, c in zip(tokenizer.all_language_tokens, tokenizer.all_language_codes, strict=False)
        }
...
class MaximumLikelihoodRanker(SequenceRanker):
    ...
    def rank(self, tokens: list[list[list[int]]], sum_logprobs: list[list[float]]):
        def scores(logprobs, lengths):
            result = []
            for logprob, length in zip(logprobs, lengths, strict=False):
        ...
        return [np.argmax(scores(p, l)) for p, l in zip(sum_logprobs, lengths, strict=False)]
...
class DecodingTask:
    ...
    def run(self, mel: mx.array) -> list[DecodingResult]:
        ...
        if self.options.task == "lang_id":
            return [
                ...
                for features, language, probs in zip(
                    audio_features, languages, language_probs, strict=False
                )
            ]
        ...
        tokens: list[list[int]] = [t[i] for i, t in zip(selected, tokens, strict=False)]
        texts: list[str] = [tokenizer.decode(t).strip() for t in tokens]

        sum_logprobs: list[float] = [lp[i] for i, lp in zip(selected, sum_logprobs, strict=False)]
        avg_logprobs: list[float] = [
            lp / (len(t) + 1) for t, lp in zip(tokens, sum_logprobs, strict=False)
        ]
