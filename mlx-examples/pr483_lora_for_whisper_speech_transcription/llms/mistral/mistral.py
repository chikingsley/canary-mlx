...
def generate(prompt: mx.array, model: Mistral, temp: float | None = 0.0):
    def sample(logits):
        if temp == 0:
...
if __name__ == "__main__":
    ...
    tokens = []
    for token, ntoks in zip(generate(prompt, model, args.temp), range(args.max_tokens), strict=False):
        tokens.append(token)
        if ntoks == 0:
            mx.eval(tokens)
            toc = time.time()
