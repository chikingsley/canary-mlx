...
class MLP(nn.Module):
    def __init__(self, n_layers: int, d_in: int, d_hidden: int, d_out: int):
        ...
        self.layers = [
            nn.Linear(idim, odim)
            for idim, odim in zip(layer_sizes[:-1], layer_sizes[1:], strict=False)
        ]
...
class RealNVP(nn.Module):
    ...
    def log_prob(self, x: mx.array):
        ...
        for mask, conditioner in zip(self.mask_list[::-1], self.conditioner_list[::-1], strict=False):
    ...
    def sample(
        ...
        for mask, conditioner in zip(
            self.mask_list[:n_transforms], self.conditioner_list[:n_transforms], strict=False
        ):
            x, _ = MaskedCoupling(
                mask, conditioner, AffineBijector
